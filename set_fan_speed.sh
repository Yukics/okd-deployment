...
### CONFIG GOES HERE
IP="$(hostname)-ilo.yuki.es"
SCREEN_NAME=iLO4fansession
SSH_USER=Administrator # You should set up a different user than Administrator in iLO, no extra permissions are needed for the user.
SSH_PASSWORD=CHANGEME # Set to the location of the SSH key for the above user.

PID_NUMBER=50  # Set here the number of PIDs that fan info reports in the algorithms table.
MIN_SPEED=500 # Set the minimum speed the fans can run at (format is XX.XX°C = XXXX)
MIN_SPEED_ADJ=5
MAX_SPEED_ADJ=60
### END CONFIG

apt-get install -y sshpass screen > /dev/null
...
if ! screen -list | grep -q "$SCREEN_NAME"; then
        ...
        screen -S $SCREEN_NAME -X stuff "sshpass -p $SSH_PASSWORD ssh ${SSH_USER}@${IP} -o StrictHostKeyChecking=no -o PubKeyAcceptedKeyTypes=+ssh-rsa -o HostKeyAlgorithms=+ssh-dss -o KexAlgorithms=+diffie-hellman-group14-sha1 -o LocalCommand='fan info'"`echo -ne '\015'`
...
screen -S $SCREEN_NAME -X stuff "fan p 0 min 63"`echo -ne '\015'`
sleep .5
screen -S $SCREEN_NAME -X stuff "fan p 1 min 63"`echo -ne '\015'`

# Set maximum speed for the fans
for ((i=0;i<=5;i++)); do
        screen -S $SCREEN_NAME -X stuff "fan p $i max $MAX_SPEED_ADJ"`echo -ne '\015'`
        sleep .5
done
